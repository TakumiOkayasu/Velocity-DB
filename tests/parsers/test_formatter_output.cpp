...
int main() {
    ...
    std::string complexSQL =
        ...
        "case when u.status='active' then 'アクティブ' when u.status='pending' then '保留中' else '無効' end as status_label,"
    ...
    std::cout << "=== Original SQL ===" << std::endl;
    std::cout << complexSQL << std::endl << std::endl;

    std::cout << "=== Formatted SQL ===" << std::endl;
    std::string formatted = formatter.format(complexSQL);
