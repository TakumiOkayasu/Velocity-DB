# TODO / Future Enhancements

このファイルはVelocity-DBの残タスクを管理します。

## 🚨 Claude Code向けルール 🚨

1. **作業開始前に必ずこのファイルを確認**
2. **タスク完了後は該当項目を削除または「完了済み」セクションに移動**
3. **新しいタスクが発生したらこのファイルに追記**

---

## 🔴 高優先度（UX向上に直結）

（現在なし - 中優先度から昇格候補を検討）

## 🟡 中優先度（機能拡張）

1. **データ比較機能** ★★★★
   - 内容: 2つのテーブルまたはクエリ結果の差分表示
   - DataGrip: テーブル比較ツール

2. **スキーマ比較・移行スクリプト生成** ★★★★★
   - 内容: 2つのスキーマを比較し、差分DDLを自動生成
   - 影響: DB移行作業の効率化

3. **リファクタリング機能** ★★★★★
   - 内容: テーブル/カラム名変更時に参照箇所を追跡・一括変更
   - DataGrip: Rename refactoring

4. **データ表示モードの追加** ★★★
   - 内容: Tree表示、Transpose(行列入替)表示
   - DataGrip: Table/Tree/Text/Transpose の4モード

5. **UIを本家DataGripに近づける** ★★ ([#56](https://github.com/TakumiOkayasu/Velocity-DB/issues/56))
   - 内容: カラースキーム、アイコン、レイアウトの改善
   - 継続的なタスク

## 🟢 低優先度（配布・運用フェーズ）

1. **インポート機能** ★★★
   - 内容: CSV, JSON等からテーブルへデータインポート
   - DataGrip: Import Data機能

2. **DDLデータソース対応** ★★★★
   - 内容: SQLファイルをデータソースとして扱い、スキーマ情報を抽出
   - DataGrip: DDL Data Source

3. **VCS統合** ★★★★★
   - 内容: Git連携でスキーマ変更履歴を管理
   - DataGrip: Version Control integration

4. **インストーラーの用意** ★★ ([#34](https://github.com/TakumiOkayasu/Velocity-DB/issues/34))
   - 内容: WiXまたはInno Setupでインストーラー作成

5. **自動更新機能** ★★★
   - 内容: 新バージョンのチェック・自動更新

## 📋 技術的改善（パフォーマンス・品質）

1. **接続プールの最適化** ★★★
   - 実装場所: `backend/database/connection_pool.cpp`
   - 改善: タイムアウト設定、ヘルスチェック、接続再利用の改善

2. **エラーハンドリングの改善** ★★
   - ~~内容: より詳細なエラーメッセージとリカバリー提案~~
   - 完了: マルチステートメント実行時のエラー位置表示
   - 残: Fix with AI的な修正提案

3. **変更プレビュー機能** ★★
   - 内容: INSERT/UPDATE/DELETE実行前にDML文をプレビュー
   - DataGrip: Preview pending changes

4. **フラグメント・イントロスペクション** ★★★★
   - 内容: DDL実行後に影響を受けたオブジェクトのみを再読み込み
   - DataGrip 2024.3: Fragment introspection

---

## ✅ 完了済み

- **DBを右クリックで閉じる** - ツリーのDB接続ノードを右クリック→「接続を閉じる」
- **環境別ツリーカラー表示** - 開発(緑)/ステージング(黄)/本番(赤)でツリーノードに色付け
- **コード補完の強化** - テーブル名、カラム名、エイリアスの文脈依存補完
- **マルチDB対応（PostgreSQL, MySQL）** - UI・バックエンド基盤実装（ODBC経由）
- **クエリ結果のソート** - カラムヘッダークリックで昇順/降順ソート
- **値エディタ (Value Editor)** - 長文やJSON等をポップアップで編集 (Shift+Enter)
- **関連レコードへのジャンプ (Related Rows)** - FK列からリンク先テーブルへ移動 (F4)
- **行の複製 (Clone Row)** - Ctrl+Dで選択行を複製して新規行として追加
- **ツリーノードのコンテキストメニュー** - SELECT文コピー、カラム一覧コピー、データを開く等
- **テーブルデータの更新ボタン** - データビュー画面のリフレッシュ機能 (#143)
- **SSH接続でDBにアクセス** - SSHトンネル経由でリモートDB接続 (#137)
- **グリッド列幅の自動調整** - データ型に応じた列幅計算、ウィンドウサイズ独立
- **SQLフォーマット** (Ctrl+Shift+F) - `backend/parsers/sql_formatter.cpp` で実装済み
- **SQLキーワード大文字変換** - Ctrl+Shift+Fでフォーマットと同時に実行
- **SQLキーワード外部ファイル化** - `config/sql_keywords.txt` で管理
- **複数SQL文の実行** - タブ式の結果表示で実装済み
- **インライン編集** - セル編集機能実装済み
- **履歴からのSQL実行機能** - ダブルクリックで実行 (b899ab6)
- **Ctrl+Wでタブを閉じる** - `MainLayout.tsx:233-235` で実装済み
- **クエリブックマーク機能** - クエリ保存/読み込み機能 (a579f1f)
- **複数SQL文の個別実行** - カーソル位置のステートメントのみ実行 (Ctrl+Enter)
- **クエリ結果のローカルフィルタ** - 各列にフィルタ入力欄（クライアントサイドフィルタ）
- **エクスポート形式の拡張** - Markdown, HTML, SQL INSERT文対応
- **クエリキャンセル機能の改善** - 停止ボタン有効化、ResultGridキャンセルボタン追加、Escキー対応
- **マルチステートメントエラー位置表示** - 何番目のstatementで失敗したかをエラーメッセージに表示
- **本番環境リードオンリーモード** - グリッド編集の多層防御（UI disabled + Hook guard + DMLブロック）
