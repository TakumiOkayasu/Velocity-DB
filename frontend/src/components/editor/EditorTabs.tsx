...
export function EditorTabs() {
  const queries = useQueries();
		...
		const fileDrop = useCallback(
			(e: React.DragEvent) => {
				if (!e.dataTransfer.types.includes("Files") || dragIndex !== null)
					return;
				e.preventDefault();
		...
		const createERDiagram = useCallback(() => {
			const existingNames = new Set(
				queries.filter((q) => q.isERDiagram).map((q) => q.name),
			);
			let idx = 1;
			let name = "ER図";
						...
						{menuOpen && (
							<div className={styles.overflowMenu}>
								{queries.map((query) => (
									...
									>
										{query.isERDiagram ? ERDiagramIcon : SqlIcon}
										<span className={styles.overflowMenuItemName}>
											{stripBrackets(query.name)}
										</span>
									</button>
								))}
